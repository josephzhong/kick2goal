...
class parallel_env(ParallelEnv):
    ...
    def can_kick(self, robot: AgentID) -> bool:
    ...
    def get_obs(self, robot: AgentID, include_history=False) -> List[float]:
        robot_loc = self.robots[robot]
        obs: List[float] = []
    ...
    def reset(
        ...
        self.robots: Dict[AgentID, List[float]] = {
            0: [np.random.uniform(-4000, 4000), np.random.uniform(-3000, 3000), 0],
        }
        self.ball: List[float] = [np.random.uniform(-4000, 4000), np.random.uniform(-3000, 3000), 0, 0]
