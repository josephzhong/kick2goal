...
class KickToGoalGym(gym.Env):
    ...
    def __init__(self, render_mode="rgb_array", seed=None, episode_length=1000, goal_reward=100, varying_init_state=False):
        ...
        self.init_ball_to_goal_angle_score = 0.0
        self.init_ball_to_goal_distance_score = 0.0
        ...
        if self.varying_init_state:
            ...
            if angle <= np.arctan((self.Field_width / 2) / self.Field_length):
                ...
                distance_min = ((self.Field_length - self.min_init_distance_to_goal) / np.cos(
                    angle)) * self.init_ball_to_goal_distance_score + self.min_init_distance_to_goal / np.cos(angle)
                distance_max = ((self.Field_length - self.min_init_distance_to_goal) / np.cos(
                    angle)) * (self.init_ball_to_goal_distance_score + 0.1) + self.min_init_distance_to_goal / np.cos(angle)
                distance = self.rng.uniform(distance_min, distance_max)
            else:
                if angle >= np.arctan((self.Field_width / 2) / (self.Field_length / 3)):
                    distance_min = self.min_init_distance_to_goal
                    distance_max = (self.Field_width / 2 ) / np.sin(angle)
                    distance = self.rng.uniform(distance_min, distance_max)
                else:
                    ...
                    distance_min = ((self.Field_width / 2 - self.min_init_distance_to_goal) / np.sin(angle)) * self.init_ball_to_goal_distance_score + self.min_init_distance_to_goal / np.sin(angle)
                    distance_max = ((self.Field_width / 2 - self.min_init_distance_to_goal) / np.sin(angle)) * (self.init_ball_to_goal_distance_score + 0.1)  + self.min_init_distance_to_goal / np.sin(angle)
                    distance = self.rng.uniform(distance_min, distance_max)
            # print(
            ...
            ball_x = np.max((np.min((ball_x, self.Field_length / 2)), -1 * self.Field_length / 2))
            ball_y = np.max((np.min((ball_y, self.Field_width / 2)), -1 * self.Field_width / 2))
            # print(angle)
            # print(distance)
